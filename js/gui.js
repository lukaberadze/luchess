...
function CheckResult() {
	if(GameBoard.fiftyMove >= 100) {
		 $("#GameStatus").text("თამაში ფრეა 15 სვლის წესით "); 
		 return BOOL.TRUE;
	}
	
	if (ThreeFoldRep() >= 2) {
     	$("#GameStatus").text("თამაში ფრეა 3 განმეორებითი სვლით"); 
     	return BOOL.TRUE;
    }
	
	if (DrawMaterial() == BOOL.TRUE) {
     	$("#GameStatus").text("თამაში ფრეა არასაკმარისი ფიგურების გამო"); 
     	return BOOL.TRUE;
    }
	...
	if(InCheck == BOOL.TRUE) {
		...
		$("#GameStatus").text("თამაში ფრეა Stalemate");return BOOL.TRUE;
	}
