...
function UpdateDOMStats(dom_score, dom_depth) {
	...
	var scoreText = "ქულა: " + (dom_score / 100).toFixed(2);
	if(Math.abs(dom_score) > MATE - MAXDEPTH) {
		scoreText = "ქულა: მატი " + (MATE - (Math.abs(dom_score))-1) + " მოძრაობაში";
	}
	
	$("#OrderingOut").text("სიზუსტე: " + ((SearchController.fhf/SearchController.fh)*100).toFixed(2) + "%");
	$("#DepthOut").text("ხარისხი: " + dom_depth);
	$("#ScoreOut").text(scoreText);
	$("#NodesOut").text("Nodes: " + SearchController.nodes);
	$("#TimeOut").text("დრო " + (($.now()-SearchController.start)/1000).toFixed(1) + "s");
	$("#BestOut").text("ბოლო სვლა: " + PrMove(SearchController.best));
}
